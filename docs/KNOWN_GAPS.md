...
| **F2/F3 was verified against `memory-ledger`, not real Fabric** - the operator's Fabric CA registrar credential was lost between sessions and this session's permission policy refused to reissue it (a secret-store write). Real-chaincode writes and the custody-transfer-receiver auto-wrap are unconfirmed live | OPEN, needs the registrar restored, then a re-run | DECISIONS D-059, docs/features/f2-f3-envelope-encryption-key-management.md |
...
Phase 3 built: D1, D2, D3, E1, E2, E3, A2 (2026-09-22 - all of Phase 3). Still open: D4 (optional). Phase 4 COMPLETE: G3, H1, H2, H3, H4, A6 (2026-09-22). Phase 5 in progress: F4 audited (2026-09-22, no violation found); F2, F3 built and verified live against memory-ledger (2026-09-22, real-Fabric confirmation still open, see section B); F5, I1, L1-L3, K2 still open. A4 (admin user management) and A5 (case-level access) appear in no build phase; users exist
