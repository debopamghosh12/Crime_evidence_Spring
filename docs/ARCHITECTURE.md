...
## 21. As built (Phase 5, L2/L3): integration tests and Docker Compose, 2026-09-22

Full account: docs/features/l2-l3-testcontainers-and-compose.md, DECISIONS D-066/D-067.

- **L2**: new `integration/EvidenceRegistrationIntegrationTest` - real Postgres (Testcontainers,
  `@ServiceConnection`), mocked Fabric (`memory-ledger`, already built), mocked IPFS (`FakeIpfsClient`, already
  built, wired in via a `@TestConfiguration` `@Primary` bean - not WireMock, despite FEATURE_LIST naming it;
  `HttpIpfsClientTest` already covers `HttpIpfsClient`'s HTTP-level behaviour with a plain JDK `HttpServer`).
  Login -> create a case -> register DIGITAL evidence -> read back -> verify(), through the real HTTP surface,
  real security filter chain and a real database with real Flyway migrations actually running - coverage a
  mocked-repository unit test cannot offer.
- **L3**: `Dockerfile` (multi-stage, this project's own Maven Wrapper) + `docker-compose.yml`
  (backend+postgres+ipfs, ipfs run `--offline` per C-09) + `.env.example`/`.env`. Fabric deliberately excluded -
  an owner-requested risk assessment (grounded in `fabric-samples/test-network/network.sh`'s own 684-line,
  multi-stage, retry-looped bootstrap, and this session's own A2 CA/wallet fragility) concluded real risk of
  sinking time on a final-year-project timeline where a working WSL-hosted Fabric network already exists; the
  owner approved the fallback. `docker compose up` alone (`dev,memory-ledger` profiles) is a complete,
  self-contained working demo. Verified with a genuine cold start (`docker compose down -v` then `up --build`,
